...
async def _main():
    data = [
        {"smiles_1": key, "smiles_2": val} for key, val in catalyst_replacements.items()
    ]
    ...
    await load_into_database(
        ...
        db_path,
        identifier_columns=[
            ("smiles_1", CompoundIdentifierType.SMILES, False),
            ("smiles_2", CompoundIdentifierType.SMILES, True),
        ],
        smiles_column="smiles_2",
    )
...
def main():
    ...
    results = resolve_identifiers(
        list(catalyst_replacements.keys()),
        output_identifier_type=CompoundIdentifierType.SMILES,
        input_identifer_type=CompoundIdentifierType.SMILES,
        services=[LocalDatabase(db_path)],
    )
    for res in results:
        print(res)
